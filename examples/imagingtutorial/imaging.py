...
if realdata: 
    datafolder = '../ER5/'
    name = 'hops_' + day + '_' + source + '+netcal'
else:
    datafolder = './results/'
    name = 'simobs'
...
if source == 'J1924-2914':
    zbl = 3.4                    # How much flux we believe lives in the compact component (Jy)
    zbl_tot = 3.4                # True zero baseline values as measured by ALMA (Jy)
    ...
    fov = 200*eh.RADPERUAS       # field of view of the reconstructed image
elif source == 'M87':
    zbl = 0.8                    # How much flux we believe lives in the compact component (Jy)
    zbl_tot = 1.2                # True zero baseline values as measured by ALMA (Jy)
    sys_noise = 0.01             # Systematic noise added to visibilities to account for (e.g.,) leakage
    prior_fwhm = 50*eh.RADPERUAS # Gaussian prior size
...
systematic_noise = {'AA':0.01, 'AP':0.01, 'AZ':0.05, 'LM':0.15, 'PV':0.05, 'SM':0.01, 'JC':0.01} # systematic noise on a priori amplitudes
...
# create a file identifier for saving out results
...
if source == 'M87':
    ...
    for repeat in range(3):
        caltab = eh.self_cal.self_cal(obs_sc.flag_uvdist(uv_max=2e9), gausspriorLMT, sites=['LM','LM'], method='vis', ttype='nfft', processes=4, caltable=True, gain_tol=1.0)
        obs_sc = caltab.applycal(obs_sc, interp='nearest', extrapolate=True)
else:
    print("Rescaling LMT Baselines")
    for d in obs_sc.data:
        if d['t1'] == 'LM' or d['t2'] == 'LM':
            for j in range(-8,0):
                d[j] *= LMT_rescale
...
#============= ROUNDS OF IMAGING AND SELF-CAL =============#
...
obs_sc = eh.self_cal.self_cal(obs_sc, im1, method='phase', ttype='nfft')
...
# Self calibrate to the previous model (phase-only)
obs_sc = eh.self_cal.self_cal(obs_sc, im2, method='phase', ttype='nfft')
...
# save the gain plots
ct = eh.self_cal.self_cal(obs, im2, method='both', ttype='nfft', processes=0, caltable=True)
